...
def download_file(url, dest_path):
    with requests.get(url, stream=True) as response:
        ...
        with Progress() as progress:
            task = progress.add_task(f"[cyan]Downloading {os.path.basename(dest_path)}...", total=total_length)
...
def main():
    ...
    parser.add_argument("output_dir", nargs="?", default="models", help="Output directory (default: ./models)")
    ...
    if os.path.exists(dest_path):
        console.print(f"[yellow]Model {model} already exists at {dest_path}. Skipping download.[/yellow]")
        return
